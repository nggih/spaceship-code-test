...
async def interpret_question(
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="The free AI model timed out.") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=503, detail="The AI service is unavailable.") from exc
    ...
    if response.status_code >= 400:
        raise HTTPException(status_code=502, detail="AI interpretation request failed.")
    try:
        payload = response.json()
        content = payload["choices"][0]["message"]["content"]
        plan = AnalysisPlan.model_validate_json(content)
        model = payload.get("model", request_body["model"])
